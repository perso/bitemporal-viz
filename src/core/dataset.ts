...
export type Dataset = { readonly tables: readonly Table[] };
...
export function buildDataset(sources: readonly SourceFile[]): Dataset {
  const entities = sources.map((file) => tableNameFromFile(file.name));
  const tables = sources.map((file, slot) => buildTable(file, slot, entities));
  const keyed = tables.filter((table) => table.keyColumn !== null);
  const unkeyed = tables.filter((table) => table.keyColumn === null);
  return { tables: [...keyed, ...unkeyed] };
}

function buildTable(file: SourceFile, slot: number, entities: readonly string[]): Table {
  const name = tableNameFromFile(file.name);
  try {
    const { header, records } = parseCsv(file.text);
    const timeColumns = detectTimeColumns(header);
    ...
    const versions = records.map((record, index) => buildVersion(shape, record, index));
    return { ...shape, slot, versions };
  } catch (error) {
    if (error instanceof Error) throw new Error(`${file.name}: ${error.message}`, { cause: error });
    throw error;
  }
}
...
function buildVersion(table: TableShape, record: CsvRecord, index: number): Version {
  ...
  if (version.validFrom === OPEN || version.techFrom === OPEN) {
    throw new Error(`row ${index + 2}: start times must not be empty`);
  }
  return version;
}
